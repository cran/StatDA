...
Statistical data analysis is about studying data graphically or analytically. But the point of view of many scientists is, that only with statistical analysis an established statement can be made. The problem is, that they ignore the power of a graphical representation of the data. Another problem is that many scientist in earth and environmental sciences are not well trained in mathematics or statistics.

This means that it is useful to produce informative graphics und to avoid mathematical formulae as much as possible. This sounds very easy but especially in the case of environmental data this can be very difficult. The reason is because many statistical tools are developed for normally distributed data but this situation only holds in rare cases. Therefore the user has to transform the data to normal distribution before useful plots can be made (for example boxplots). Then there is the problem that the scaling differ from the orginal data which means the user has to back-transform the data. This easy example illustrates that it can be difficult to produce a plot for environmental data.

Another problem is that it is necessary to compare the concentration of many different elements. This often results in the problem of visualizing more than 3 variables. This means that it is important to find a possibility to reduce the dimension or to create a graphic for more than 3 dimensions. We will focus on the second solution because our main focus is on the graphical representation of the data. 

This package was written to help the user in statstically analyzing environmental data. All functions of \StatDA are written to produce the plots or tabels in the book ``Statistical Data Analysis Explained'' \footnote{C. Reimann, P. Filzmoser, R.G. Garrett, R. Dutter; 2007}. The graphics in the book can be reproduced with the R-code from the homepage
\begin{quote}
http://www.statistik.tuwien.ac.at/StatDA/R-scripts
\end{quote}
All plots are computed with the data set of the Kola Ecogeochemistry Project (Reimann et al., 1998) in the European Arctic. The input data are included in the package. The structure of this vignette follows the book and the graphics are part of the book. The explanations are the basic informations from the different chapters. For further descriptions we would recommend the book. 


\chapter{Graphics to Display the Data Distibution}
The Kola Project includes chemical analysis for ca. 50 chemical elements at about 600 sample sites. These huge amount of input data needs to be summarised. First of all it is important to know which distributions the data follow. Therefore some basic statistical plots can be made. The easiest plots are the histogram, density plot, scatterplot and a boxplot. The graphics can look very different for the different samples. For example the distribution can be symmetrical or asymmetrical. The data can fall into two groups or single extreme outliers can appear.
...
First of all a short explanation of the ECDF. It is not very difficult to understand this function because the function jumps witch each data point by $1/n$, where $n$ is the number of data points. The x-axis shows the variable values and the y-axis the corresponding probabilities of the empirical cumulative distribution function (values between 0 and 1).
...
data(moss)
out<-sumstats(moss[,-c(1:3)])

@

<<label=tab1,echo=FALSE,results=tex,width=9,height=9>>=
library(xtable)
xtable(out,caption="MIN = minimum value, MAX = maximum value reported, the other values are descriped above", label="tab1")
...
The spatial component makes a big difference between data sets collected by earth scientist and other data sets. The data sets were taken somewhere on earth and are characterised by coordinates. Therefore the location of the sample must be included in the analysis of the data. This is the reason why it is necessary to show the statistical structure in apporpriate maps. As one can imagine, many different methods exist to show this structure. Some of them look informative and others are featureless. It is surprising that scientists pay so little attention to the preparation of geochemical maps. There is no common effective method to display environmental data on a map. Our main focus in this chapter is to present some different mapping methods.

\section{Mapping Geochemical Data with Proportional Dots}
This method is very easy to understand and results in a method which is easy to read. It is also one of the most popular ways for black and white maps. The main idea is that the absolute value of the sample is important. All different values will be represented in dots. If one takes linearly growing dots there can be two problems. The first one is that too many dots are similar in size and it is difficult to distinguish different values. On the other hand it is possible that the maximum is very far away from the rest of the data and then one huge dot will appear and the remaining dots are very small.

An alternative is exponentially growing dots. It is true that the problem with one huge dot can also appear. Therefore one has to make a little trick. Before one can start to calculate the dots he must make the assumption that, for example, for the smalles 10 percent and the biggest 1 percent of the empirical cumulative destribution function the symbol size is constant. In between the dots grow exponentiallay with the result that the dot size slows down near the detection limit and becomes steeper towards the larger values. 

Figure~\ref{fig:two} shows the map of the Kola Project area of As in C-horizon with exponential growing dots.
...
Another possibility is to use a boxplot instead of percentiles. The problem is the same as mentioned in the previous chapter. The boxplot identifies a reliable number of outlier if the data distribution is symmetric. Therefore it can be necessary to use another version of the boxplot such as the log-boxplot.
...
Kriging is a statistically optimised estimator for interpolation. For kriging exist different methods like block kriging (interpolation in a whole block) and point kriging (interpolation for any location in the space). The main idea is that points in the neighbourhood have more in common than points with a greater distance. The cruical point is to weight the different distances.

The basis for kriging is the (semi)variogram. This variogram displays the variance between data points at defined distances and the distance is the Euclidean distance in the survey area. The maximum on the x-axis is 30 to 50 percent of the maximum distance in the survey area. In the case of the Kola Project the maximum is about 700 km. With the Euclidean distance any direction is possible but sometimes only one direction can be interesting. Therefore the distances can be calculated in only one direction, e.g. north-south or east-west. The omnidirectional variogram is a kind of average over all possible directions. In Figure~\ref{fig:five} (left) the variogram for different directions is displayed and the right graphic shows the so called sphericalvariogram that can be used as a model for kriging.

\newpage
Only the important part of the code is shown.
...
For further information we recommand the book. Now we have done all of the preparition to create a kriging plot of the map for the Kola Project. As mentioned above it is possible to make kriging for a block or a single point. In our case we plot the interpolation of the map with block kriging. If the blocks are visible or not depends on the size of the block.
...
@
\end{center}
\caption{Block kriging (20 x 20 km blocks) of As in C-horizon. The semivariogram in Figure~\ref{fig:five} (right) is used for kriging.}
\label{fig:six}
\end{figure}
...
@
\end{center}
\caption{Points in the map used for the east-west belt (left). Plot whicht shows the decreasing of the Cu concentration with bigger distance to Monchegorsk.}
\label{fig:seven}
\end{figure}
...
In Figure~\ref{fig:eight} the reference point (Nikel/Zapojarnij) is in the north-west and the subarea goes to Monchegorsk in the south-east. In the plot on the right side it is visible that near Nikel-Zapojarnij and Monchegorsk the concentration of Cu is higher than between the two places. The width of the peak says that the emissions from Monchegorsk travel farther.
...
axis(2,at=log10(a<-sort(c((10^(-50:50))%*%t(c(2,5,10))))),labels=a)
lines(smooth.spline(distanc/1000,log10(Cu[loc.in]),spar=
0.8),col=1,lwd=1.3)
@
\begin{figure}
\begin{center}
...
@
\end{center}
\caption{Density traces of Al in all four parts of the ecosystem}
\label{fig:ten}
\end{figure}

Another interesting graphic is a CP-Plot (Cumulative Probability Plot) of the data set. This can be made in the following way where every layers of the ecosystem is displayed with a differnt symbol. It can be a better solution to plot them with different colours.
...
@
\end{center}
\caption{Log-boxplot (because of the problems mentioned in Chapter 2) of Al in the four different parts of the ecosystem.}
\label{fig:twelve}
\end{figure}
...
But all methods are not really robust and therefore we need a robust estimation of the correlation. One possibility is the Minimum Covariance Determinant (MCD) which searches for the most compact ellipse containing e.g. 50 percent of the data points. The user can choose any percentage between 50 and 100 percent. If the user goes near to the 100 percent the robustness of the estimator is lost. Figure~\ref{fig:thirteen} demonstrates that the Pearson correlation and the MCD estimation deliver different results.
...
\item Boxes: First the variables undergo a hierarchical cluster analysis. Each cluster corresponds to one side of the box. This method is only applicable when the variables can be grouped in three clusters. The dimension of each axis is proportional to the sum of values.
\item Trees: The computation is similar to boxes. The advantage is that the variables are not forced to be grouped in three clusters. The result is the same as for a dendogram. The height of the battlements or length of the branches represent the values of the variables.
\end{itemize}
...
The detection of data outliers and unusual data structures is very important in the statistical analysis of data. The basis for an outlier is the location and spread of the data. Outliers typically have a large distance to the center of the sample and therefore it is important to find a threshold, dividing the observations into background and outlier. How to find such a threshold has received much attention in geochemical literature.

When we compares univariate and multivariate outlier detection we can see that there is a difference. In the univariate case a box (for example all observation except the lower and upper 2 percent) is defined. All values outside the box are characterized as outliers. The problem is that in this case the multivariate, often elliptical, structure of the data set is ignored. Therefore a better solution is to estimate ellipses to defined quantiles. All values outside the ellipse are outliers. Now there appears a new problem which is, how to estimate the central location and covariance. We have talked about the difference between robust and non-robust estimation in Chapter 3. Different methods result in different ellipses and therefore different characterizations.
...
\end{itemize}
We have talked about the problem of outliers in a regression model because they can influence the regression line or hyperplane. Therefore it is interesting to find a robust method for estimating the regression coefficients. As mentioned above, a possibility is to use least trimmed sum of squares (LTS) which downweights the larger residuals. After fitting a LTS model we can make some checks if the assumptions are fulfilled. With a plot, where the x-axis represents the robust Mahalanobis distances and the y-axis the value of the residuals, it is possible to identify outliers. 
